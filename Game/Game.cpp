...
====================================== 
*/
void Game::DrawPiece (int pX, int pY, int pPiece, int pRotation, int blockSize)
{
	...
	for (int i = 0; i < PIECE_BLOCKS; i++)
	{
		for (int j = 0; j < PIECE_BLOCKS; j++)
		{
			...
			if (mPieces->GetBlockType (pPiece, pRotation, j, i) != 0)
				mIO->DrawRectangle	(mPixelsX + i * blockSize,
									mPixelsY + j * blockSize,
									(mPixelsX + i * blockSize) + blockSize - 1,
									(mPixelsY + j * blockSize) + blockSize - 1,
									mColor);
		}
	}
}
...
void Game::DrawScene ()
{
	...
	DrawPiece (mPosX, mPosY, mPiece, mRotation, BLOCK_SIZE);					// Draw the playing piece
	DrawPiece (mNextPosX, mNextPosY, mNextPiece, mNextRotation, 2);	// Draw the next piece
}
